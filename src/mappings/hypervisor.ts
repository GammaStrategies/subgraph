...
export function handleRewardClaimed(event: RewardClaimed): void {
  ...
  let rewardedToken = RewardedToken.load(
    event.params.vault.toHexString() + "-" + hypervisor.rewardToken
  );
  if (rewardedToken == null) {
    rewardedToken = createRewardedToken(
      event.params.vault,
...
export function handleVaultFactoryRegistered(
  event: VaultFactoryRegistered
): void {
  let hypervisor = getOrCreateStakingHypervisor(event.address.toHex());
