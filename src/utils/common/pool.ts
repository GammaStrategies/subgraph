...
export function processSwap(
	...
	const elapsedSinceLastHypervisorRefresh = block.timestamp.minus(pool.lastHypervisorRefreshTime)
	if (elapsedSinceLastHypervisorRefresh > hypervisorUpdateIntervalSeconds) {
		...
		for (let i = 0; i < hypervisors.length; i++) {
			...
			const hypervisorDayData = updateAndGetUniswapV3HypervisorDayData(hypervisors[i])
