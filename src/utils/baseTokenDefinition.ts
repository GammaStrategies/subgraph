...
export class BaseTokenDefinition {
  ...
  static polygon(): TypedMap<string, BasePool> {
    ...
    lookup.set(USDT, { pathIdx: [0], path: [USDC_USDT], priority: 4 });
    lookup.set(DAI, { pathIdx: [0], path: [DAI_USDC], priority: 3 });
    lookup.set(WMATIC, { pathIdx: [1], path: [WMATIC_USDC], priority: 2 });
    lookup.set(WETH, { pathIdx: [0], path: [USDC_WETH], priority: 1 });
